    ...
    try {
      ...
      if (policyTemplate && packageInfo.name) {
        ...
        if (requestingGroup) {
          ...
          const existingPolicies = await soClient.find<PackagePolicySOAttributes>({
            type: PACKAGE_POLICY_SAVED_OBJECT_TYPE,
            filter: `${PACKAGE_POLICY_SAVED_OBJECT_TYPE}.attributes.cloud_connector_id:"${existingCloudConnectorId}"`,
            perPage: 1,
          });

          if (existingPolicies.total > 0) {
            const existingPolicy = existingPolicies.saved_objects[0].attributes;
            const existingPackageName = existingPolicy.package?.name;
            const existingPolicyTemplate = existingPolicy.inputs?.find(
              (input) => input.policy_template
            )?.policy_template;
            ...
            if (existingPackageName && existingPolicyTemplate) {
              ...
              if (connectorGroup && connectorGroup !== requestingGroup) {
                throw new CloudConnectorCreateError(
                  `Cloud connector ${existingCloudConnectorId} belongs to policy group "${connectorGroup}" and cannot be reused by integration "${packageInfo.name}/${policyTemplate}" in policy group "${requestingGroup}"`
                );
              }

              if (!connectorGroup) {
                throw new CloudConnectorCreateError(
                  `Cloud connector ${existingCloudConnectorId} is used by integration "${existingPackageName}/${existingPolicyTemplate}" which is not in any recognized policy group and cannot be reused by "${packageInfo.name}/${policyTemplate}"`
                );
              }
            }
          }
