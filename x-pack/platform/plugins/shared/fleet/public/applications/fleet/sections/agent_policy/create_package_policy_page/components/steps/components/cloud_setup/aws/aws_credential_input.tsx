...
import { EuiFieldText, EuiFormRow, EuiLoadingSpinner, EuiSpacer, EuiText } from '@elastic/eui';
...
// const getInputVarsFields = (input: NewPackagePolicyInput, fields: AwsCredentialsFields) =>
...
const getVarsFields = (credentials: AwsFormCredentials, fields: AwsCredentialsFields) => {
  return Object.entries(fields).map(([id, field]) => {
    return {
      id,
      label: field.label,
      type: field.type || 'text',
      value: credentials.vars[id],
      dataTestSubj: field.dataTestSubj,
      isSecret: field.isSecret,
    } as const;
  });
